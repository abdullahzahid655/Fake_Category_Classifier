...
\begin{tikzpicture}[
  ...
  \node[box,right=0.85cm of raw]  (lc)  {Lowercase};
  \node[box,right=0.85cm of lc]   (rm)  {Remove\\HTML/URLs};
  \node[box,right=0.85cm of rm]   (rp)  {Remove\\Punct/Digits};
  \node[box,below=0.3cm of rm]    (tok) {Tokenize};
  \node[box,left=0.85cm of tok]   (sw)  {Remove\\Stopwords};
  \node[box,left=0.85cm of sw]    (lem) {Lemmatize};
  \node[box,left=0.85cm of lem,fill=accent!15,draw=accent] (out) {Clean Text};
  \draw[arrow] (raw) -- (lc);
  \draw[arrow] (lc) -- (rm);
  \draw[arrow] (rm) -- (rp);
...
\begin{center}
  \includegraphics[width=0.62\textwidth]{../outputs/wordclouds.png}
\end{center}
\vspace{-4pt}
...
%--- Dashboard Part 1: Class Distribution + Model Accuracy ---
\begin{frame}{Results — Class Distribution \& Model Accuracy}
\begin{center}
  \includegraphics[width=0.95\textwidth]{../outputs/dashboard_part1.png}
\end{center}
\vspace{-4pt}
\footnotesize\textit{Perfectly balanced classes (30\,000 each). MLP leads at 91.8\% accuracy, closely followed by Linear SVM (91.8\%) and Logistic Regression (91.7\%). Random Forest trails at 89.7\%.}
\end{frame}

%--- Dashboard Part 2: Top-10 Words + Confusion Matrices ---
\begin{frame}{Results — Top-10 Words \& Confusion Matrices}
\begin{center}
  \includegraphics[width=0.62\textwidth]{../outputs/dashboard_part2.png}
\end{center}
\vspace{-4pt}
\footnotesize\textit{Each category has a distinct vocabulary signature. Business/World confusion is the most frequent misclassification across all models; Sports is consistently the cleanest class.}
\end{frame}

%--- Dashboard Part 3: Per-Class F1-Score Heatmap ---
\begin{frame}{Results — Per-Class F1-Score Heatmap}
\begin{center}
  \includegraphics[width=0.95\textwidth]{../outputs/dashboard_part3.png}
\end{center}
\vspace{-4pt}
\footnotesize\textit{Sports achieves the highest F1 ($\geq$0.95) across all models. Business is the hardest class (F1 0.858--0.887). MLP and Linear SVM are the most consistent top performers.}
\end{frame}
...
\begin{columns}[T]
  \column{0.55\textwidth}
    \begin{center}
    %% FIX: resizebox ensures chart never overflows column width
    \resizebox{\columnwidth}{!}{%
    \begin{tikzpicture}
      \begin{axis}[
        xbar, bar width=14pt,
        width=9.5cm, height=6.5cm,
        %% FIX: extra left margin so long y-labels (e.g. "MLP Neural Network") are not clipped
...
\begin{columns}[T]
  \column{0.58\textwidth}
    ...
    %% FIX: resizebox + row labels moved to positive x with anchor=east
    %%      so they never escape the column boundary
    \resizebox{\columnwidth}{!}{%
    \begin{tikzpicture}[
      ...
      %% FIX: row labels shifted RIGHT to x=0.05 with anchor=east (was x=-0.3, causing clipping)
      \node[font=\footnotesize,anchor=east] at (0.05,0.425) {Sci/Tech};
      \node[font=\footnotesize,anchor=east] at (0.05,1.375) {Business};
      \node[font=\footnotesize,anchor=east] at (0.05,2.325) {Sports};
      \node[font=\footnotesize,anchor=east] at (0.05,3.275) {World};
      %% World row
...
\begin{columns}[T]
  \column{0.52\textwidth}
    ...
    %% FIX: resizebox + all negative-x elements brought inside boundary
    \resizebox{\columnwidth}{!}{%
    \begin{tikzpicture}[
      ...
      %% Column headers (predicted, rotated above matrix)
      \node[hdr,rotate=30] at (0.8, 4.05)  {World};
      \node[hdr,rotate=30] at (2.45,4.05)  {Sports};
      \node[hdr,rotate=30] at (4.10,4.05)  {Business};
      \node[hdr,rotate=30] at (5.75,4.05)  {Sci/Tech};
      %% FIX: row headers moved to x=0.05 anchor=east (was x=-0.1, "True Label" was x=-0.85)
      \node[hdr,anchor=east] at (0.05,0.875) {Sci/Tech};
      \node[hdr,anchor=east] at (0.05,1.775) {Business};
      \node[hdr,anchor=east] at (0.05,2.675) {Sports};
      \node[hdr,anchor=east] at (0.05,3.575) {World};
      %% World row
      ...
      %% FIX: "True Label" moved to x=6.9 on the RIGHT (was x=-0.85, far outside left boundary)
      \node[hdr,rotate=90,anchor=center] at (6.9,2.225) {True Label};
      \node[hdr] at (3.3,4.55) {Predicted Label};
    \end{tikzpicture}%
    }% end resizebox
    \end{center}
...
\begin{columns}[T]
  \column{0.55\textwidth}
    ...
    %% FIX: resizebox keeps the bar chart within column width
    \resizebox{\columnwidth}{!}{%
    \begin{tikzpicture}
      \begin{axis}[
        ybar, bar width=22pt,
